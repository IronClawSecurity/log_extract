...
typedef struct {
    ...
    char    exclude[256];   /* empty = no exclude filter */
...
/* Check if a line passes the exclude filter (returns 0 if line should be dropped) */
...
/* Combined: check keyword + user + exclude against a text line */
